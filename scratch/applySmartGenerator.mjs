...
// Replace the fallback return in getContent with generateSmartContent
const targetFallbackRegex = /return\s*\{\s*heroTitle:\s*titleCase\(keyword\)[\s\S]*?\n\s*\};\s*\n\}/;

if (!targetFallbackRegex.test(code)) {
  console.error('Could not find fallback in getContent');
  process.exit(1);
}

code = code.replace(targetFallbackRegex, `return generateSmartContent(keyword, slug);\n}`);

// Add generateSmartContent right before getContent function
const getContentPos = code.indexOf('function getContent(keyword: string):');
if (getContentPos === -1) {
  console.error('Could not find getContent function definition');
  process.exit(1);
}

code = code.slice(0, getContentPos) + smartGenerator + '\n' + code.slice(getContentPos);

fs.writeFileSync(filePath, code, 'utf8');
console.log('Successfully added smart generator to lib/keywordPages.ts');
