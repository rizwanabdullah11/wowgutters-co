...
const keywords = JSON.parse(fs.readFileSync('scratch/cleanedKeywords.json', 'utf8'));

// Also get original keywords
const origKeywords = [
...
for (const k of [...origKeywords, ...keywords]) {
  const s = slugify(k);
...
const fileContent = `import { suburbPages } from '@/lib/suburbPageData';
...
const RAW_KEYWORDS = [
${allKw.map(k => `  ${JSON.stringify(k)},`).join('\n')}
];
...
fs.writeFileSync('lib/keywordPages.ts', fileContent, 'utf8');
console.log('Successfully regenerated lib/keywordPages.ts with total keywords:', allKw.length);
