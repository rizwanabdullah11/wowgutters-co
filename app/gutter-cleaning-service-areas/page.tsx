...
export const metadata: Metadata = buildMetadata({
  ...
  description:
    'Find professional gutter cleaning in your area. WOW Gutters Ltd covers 220+ locations across Birmingham, Coventry, Wolverhampton, Solihull, Walsall, Dudley and the West Midlands.',
  canonicalPath: '/gutter-cleaning-service-areas/',
});

export default function GutterCleaningServiceAreasPage() {
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <div className="max-w-7xl mx-auto px-4 py-14 md:py-20">
        ...
        <p className="text-lg text-slate-600 max-w-3xl mb-4 leading-relaxed">
          WOW Gutters Ltd offers professional gutter cleaning across Birmingham and the wider West
          Midlands. Select your area for local pricing, postcodes, FAQs and booking — all {sorted.length} locations
          are linked below.
        </p>
        <p className="text-slate-600 mb-4">
          Need roof cleaning instead?{' '}
          <Link href="/roof-cleaning-service-areas/" className="font-bold underline" style={{ color: colors.primary }}>
            View roof cleaning service areas
          </Link>
          .
        </p>
        <p className="mb-10">
          ...
          <ul className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-4 gap-y-2">
            {sorted.map((slug) => (
              <li key={slug}>
                ...
                >
                  Gutter Cleaning {areaDisplayName(slug)}
                </Link>
              </li>
            ))}
          </ul>
