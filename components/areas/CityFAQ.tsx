...
import type { CityGutterLandingFaq } from '@/constants/cityGutterLandingData';

export default function CityFAQ({ title, faqs }: { title: string; faqs: CityGutterLandingFaq[] }) {
  const [openIdx, setOpenIdx] = useState<number | null>(0);

  return (
    <>
      {/* Note: FAQPage schema is included in the city page's @graph structure */}
      <section
            ...
            {faqs.map((faq, i) => (
              <div
                ...
                className={`bg-[#0f172a] rounded-2xl overflow-hidden border transition-all duration-300 ${
                  openIdx === i ? 'border-white/30 shadow-lg shadow-black/20' : 'border-white/10 hover:border-white/20'
                }`}
              >
                <button
                  type="button"
                ...
                <div className={`overflow-hidden transition-all duration-300 ${openIdx === i ? 'max-h-[28rem]' : 'max-h-0'}`}>
