...
export default function RecentWork() {
  const featuredPost = workPosts[0];
  const topWorkPosts = workPosts.slice(1, 4);
  const bottomWorkPosts = workPosts.slice(4, 7);
    ...
    <section className="py-20" style={{ backgroundColor: colors.background }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Link href={`/work/${featuredPost.id}`}>
          ...
          >
            <img
              ...
              className="w-full lg:w-1/3 h-52 object-cover"
            />
            <div className="p-6 flex flex-col justify-center flex-1">
              ...
              <p className="text-xs mb-3" style={{ color: colors.textLight }}>
                Completed: {featuredPost.date} · {featuredPost.location} · {featuredPost.views} views
              </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
          {topWorkPosts.map((post, index) => (
            <Link key={index} href={`/work/${post.id}`}>
              <div
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          {bottomWorkPosts.map((post, index) => (
            <Link key={index} href={`/work/${post.id}`}>
              <div
                className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 h-full flex flex-col"
                style={{ borderTop: `3px solid ${ACCENTS[index]}` }}
        ...
        <div className="text-center">
          ...
          >
            Load More Work <ArrowRight className="w-4 h-4" />
          </button>
        </div>
      </div>
    </section>
